...
export async function generateStaticParams(): Promise<Array<{ countryCode: string }>> {
...
export default async function CountryPage({ params }: PageProps) {
  ...
  const breadcrumbs = [
    { name: 'Home', url: '/' },
    { name: 'Services', url: '/services' },
    { name: country.name, url: `/services/${country.slug}` }
  ];
  ...
  return (
    <>
      ...
      <SemanticSEOSection
        ...
        entities={[
          { name: 'Local SEO', type: 'service', link: `/services/${country.slug}` },
          { name: 'Technical SEO', type: 'service' },
          { name: 'Content Strategy', type: 'concept' },
          { name: country.name, type: 'location', link: `/services/${country.slug}` },
      ...
      >
        <EntityRelationship
          ...
          entities={[
            { id: 'local-seo', name: 'Local SEO', type: 'service', description: `Optimize for ${country.name} local searches`, link: `/services/${country.slug}` },
            ...
            ...country.cities.slice(0, 4).map(city => ({
              ...
              link: `/services/${country.slug}/${city.slug}`,
            })),
          ]}
          relations={[
